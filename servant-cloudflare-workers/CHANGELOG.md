...
- Full query string helpers [#1604](https://github.com/haskell-servant/servant/pull/1604)
  This involves a new instance `HasWorker (QueryString :> api) context`.
...
- Add instance `HasWorker (EmptyAPI :> api) context` [#1775](https://github.com/haskell-servant/servant/pull/1775)
...
- Add API docs for WorkerT [#1573](https://github.com/haskell-servant/servant/pull/1573)
...
- Merge in `servant-generic` (by [Patrick Chilton](https://github.com/chpatrick))
  ...
  and `servant-server` (`Servant.Cloudflare.Workers.Generic`).

- *servant-server* Deprecate `Servant.Utils.StaticUtils`, use `Servant.Cloudflare.Workers.StaticUtils`.
...
* Added `hoistWorker` member to the `HasWorker` class, which is `HasWorker`
  specific `enter`.
...
* Changed `HasWorker` instances for `Header` to throw 400 when parsing fails
...
* Changed `HasWorker` instances for `QueryParam` and `QueryParam` to throw 400
  when parsing fails
...
* Remove module `Servant.Cloudflare.Workers.Internal.Enter` (https://github.com/haskell-servant/servant/pull/478)
* Support GHC 8.0
...
* Add `Config` machinery (https://github.com/haskell-servant/servant/pull/327).
  ...
  typeclass `HasWorker` now take an additional parameter.
...
* Make code sample in Servant.Cloudflare.Workers complete
