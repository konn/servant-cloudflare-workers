  ...
  describe "HTTP error order"
    ...
    $ do
      ...
      it "has 400 as its sixth highest priority error" $ do
        ...
        when (both /= badParamsRes) $
          liftIO $
            expectationFailure $
              "badParams + badBody /= badParams: " ++ show both ++ ", " ++ show badParamsRes
        when (both == badBodyRes) $
          liftIO $
            expectationFailure $
              "badParams + badBody == badBody: " ++ show both
    ...
    let startsWithCustom =
          ResponseMatcher
            ...
            , matchBody = MatchBody $ \_ body ->
                if "CUSTOM!" `BSL8.isPrefixOf` body
                  then Nothing
