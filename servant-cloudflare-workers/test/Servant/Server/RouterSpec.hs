...
module Servant.Cloudflare.Workers.RouterSpec (spec) where
...
spec :: Spec
spec = describe "Servant.Cloudflare.Workers.Internal.Router" $ do
  routerSpec
  distributivitySpec
  serverLayoutSpec
...
routerSpec = do
  describe "tweakResponse" $ do
    let app' :: Application
        app' = toFetchHandler $ runRouter (const err404) router'
  ...
  describe "runRouter" $ do
    let toApp :: Router () -> Application
        toApp = toFetchHandler . runRouter (const err404)
