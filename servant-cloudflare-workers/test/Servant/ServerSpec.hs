...
module Servant.Cloudflare.WorkersSpec where
...
import Servant.Cloudflare.Workers.Experimental.Auth (
  AuthHandler,
  AuthServerData,
  mkAuthHandler,
 )
      ...
      test desc api method (status :: Int) = context desc
        ...
        $ do
          ...
          when (method == methodHead)
            $ it "HEAD returns no content body"
            $ do
              response <- THW.request method "/" [] ""
              liftIO $ simpleBody response `shouldBe` ""
          ...
          unless (status `elem` [214, 215] || method == methodHead)
            $ it "allows modular specification of supported content types"
            $ do
...
captureAllSpec = do
  ...
  describe "Servant.API.CaptureAll" $ do
    ...
    with
      ...
      $ get "/captured/foo/bar/baz"
      `shouldRespondWith` fromString (show ([] :: [Int]))
  ...
  describe "Servant.API.QueryParam" $ do
    it "allows retrieving simple GET parameters"
      ...
      $ do
        ...
        liftIO $ decode' (simpleBody response1)
          `shouldBe` Just
            alice
              { name = "bob"
              }
    ...
    it "allows retrieving lists in GET parameters"
      ...
      $ do
        ...
        liftIO $ decode' (simpleBody response2)
          `shouldBe` Just
            alice
              { name = "john"
              }
    ...
    it "parses a query parameter"
      ...
      $ do
        ...
        liftIO $ decode' (simpleBody response)
          `shouldBe` Just
            alice
              { age = 55
              }
    ...
    it "allows retrieving value-less GET parameters"
      ...
      $ do
        ...
        liftIO $ decode' (simpleBody response3)
          `shouldBe` Just
            alice
              { name = "ALICE"
              }
...
alternativeSpec = describe "Servant.API.Alternative" $ with (return $ serve alternativeApi alternativeServer) $ do
  it "unions endpoints" $ do
    ...
    liftIO $ decode' (simpleBody response)
      `shouldBe` Just alice
    response_ <- get "/bar"
    liftIO $ decode' (simpleBody response_)
      `shouldBe` Just jerry
...
miscCombinatorSpec = with (return $ serve miscApi miscServ)
  $ describe "Misc. combinators for request inspection"
  $ do
