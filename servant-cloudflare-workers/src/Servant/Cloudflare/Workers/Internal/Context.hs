...
with 'hoistWorker'.) If you don't use combinators that
require any context entries, you can just use 'Servant.Cloudflare.Workers.serve' as always.

If you are using combinators that require a non-empty 'Context' you have to
use 'Servant.Cloudflare.Workers.serveWithContext' and pass it a 'Context' that contains all
...
> import Servant.Cloudflare.Workers (type (.++))
-}
